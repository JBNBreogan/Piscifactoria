...
public class Simulador {
    ...
    /**
     * Método que inicializa el sistema, asignando el nombre del sistema, creando
    ...
    public void init() {
        ...
        this.registros = new Registros(nombreEmpresa);
        this.registros.inicio(npisc, null, new String[] { AlmacenPropiedades.LUCIO_NORTE.getNombre(),
                AlmacenPropiedades.CARPA_PLATEADA.getNombre(),
                AlmacenPropiedades.CARPA.getNombre(),
                AlmacenPropiedades.TILAPIA_NILO.getNombre(),
                AlmacenPropiedades.PEJERREY.getNombre(),
    ...
    /**
     * Método que muestra el menú de piscifactorías y permite seleccionar una de
     * ellas.
     * 
    ...
    /**
     * Método que muestra el menú de tanques y permite seleccionar un tanque,
     * mostrando un menú de los disponibles.
     * 
     * @return El número que ocupa en la lista de tanques de una pisicfactoria el
     *         tanque seleccionado
     */
    ...
    /**
     * Método que muestra el día actual, las monedas del sistema, el estado de todas
     * las piscifactorías del sistema y en caso de tener
    ...
    public void showGeneralStatus() {
        ...
        if (almacenCentral != null) {
            int comidaAnimalPorcentaje = (almacenCentral.getCapacidadComidaAnimal() != 0)
                    ? ((almacenCentral.getComidaAnimal() * 100) / almacenCentral.getCapacidadComidaAnimal())
                    : 0;
            System.out.println("Deposito de comida animal del almacen central al " + comidaAnimalPorcentaje
                    + "% de su capacidad. [" + almacenCentral.getComidaAnimal() + "/"
                    + almacenCentral.getCapacidadComidaAnimal() + "]");

            int comidaVegetalPorcentaje = (almacenCentral.getCapacidadComidaVegetal() != 0)
                    ? ((almacenCentral.getComidaVegetal() * 100) / almacenCentral.getCapacidadComidaVegetal())
                    : 0;
            System.out.println("Deposito de comida vegetal del almacen central al " + comidaVegetalPorcentaje
                    + "% de su capacidad. [" + almacenCentral.getComidaVegetal() + "/"
    ...
    /**
     * Método que muestra el estado de todos los peces del sistema el número de
     * ellos nacidos, el número de vendidos y las monedas obtenidas con esto,
    ...
    /**
     * Método que muestra una lista de los peces disponibles en el sistema y permite
     * elegir uno.
     * 
    ...
    /**
     * Método que avanza un día en el sistema, realiza el crecimiento de los peces,
     * la reproducción y la venta de peces óptimos y por último muestra un mesaje de
     * los peces vendidos
    ...
    public void nextDay() {
        ...
        this.registros.pasarDia((dias - 1), numPecesRio, numPecesMar, monedasObtenidas, monedero.getMonedas());
    }

    /**
     * Método que permite seleccionar una piscifactoria, seleccionar el tipo de
     * comida que quieres añadir, seleccionar la cantidad de comida, y la añade.
     */
    public void addFood() {
        if (almacenCentral != null) {
            anadirComidaAlm();
    ...
    /**
     * Método que muestra las piscifactorias del sistema permite elegir una, despues
     * muestra los tanques del sistema y permite elegir uno y después muestra
                    ...
                    if (Monedero.monedasSuficientes(pezEleg.getCoste())) {
                        if (pisc.pecesEnPiscifactoria() < pisc.pecesMaxPiscifactoria()) {
                            if (tank.getPeces().size() < 1) {
                                ...
                                if (pezEleg.isFemale()) {
                                    this.registros.comprarPeces(pezEleg, 'H', pisc.getTanques().indexOf(tank), pisc,
                                            pezEleg.getCoste());
                                } else {
                                    this.registros.comprarPeces(pezEleg, 'M', pisc.getTanques().indexOf(tank), pisc,
                                            pezEleg.getCoste());
                                }
                                ...
                                if (tank.getPeces().get(0).getName() == pezEleg.getName()) {
                                    if (tank.getPeces().size() < tank.getMaxPeces()) {
                                        ...
                                        stats.registrarNacimiento(pezEleg.getName());
                                        if (pezEleg.isFemale()) {
                                            this.registros.comprarPeces(pezEleg, 'H', pisc.getTanques().indexOf(tank),
                                                    pisc, pezEleg.getCoste());
                                        } else {
                                            this.registros.comprarPeces(pezEleg, 'M', pisc.getTanques().indexOf(tank),
                                                    pisc, pezEleg.getCoste());
                                        }
                                        tank.showCapacity(pisc.getTanques().indexOf(tank));
                                        break;
                                    ...
                                    System.out
                                            .println("Este pez no es válido para este tanque, introduce un pez válido");
                                }
                            }
    ...
    /**
     * Método que permite seleccionar una piscifactoría y vende todos los peces
     * adultos de esta, mostrando al final un mensaje de los peces vendidos y las
     * monedas obtenidas con ello.
     */
    public void sell() {
        try {
            Piscifactoria pisc = piscifactorias.get(selectPisc());
            ...
            this.registros.venderPeces(pecesVend, monedasOb, pisc);
            System.out.println("Piscifactoría " + pisc.getNombre() + ": " + pecesVend + " peces vendidos por "
                    + monedasOb + " monedas");
        } catch (IndexOutOfBoundsException e) {
            System.out.println("Introduce un número válido, krak");
        }
    ...
    /**
     * Método que permite seleccionar un tanque, y elimina todos los peces del mismo
    ...
    public void emptyTank() {
        try {
            ...
            tank.getPeces().clear();
            this.registros.limpiarTanque(pisc.getTanques().indexOf(tank), pisc);
        } catch (IndexOutOfBoundsException e) {
            System.out.println("Introduce un número válido, krak");
        }
    }
                    ...
                    switch (op2) {
                            ...
                            do {
                                if (tipoPisc == 1) {
                                    if (Monedero.monedasSuficientes(500 * piscifactorias.size())) {
                                        ...
                                        this.registros.comprarEdificio(new Piscifactoria(nombrePisc, CriaTipo.RIO),
                                                500 * piscifactorias.size());
                                    ...
                                    if (Monedero.monedasSuficientes(2000 * piscifactorias.size())) {
                                        ...
                                        this.registros.comprarEdificio(new Piscifactoria(nombrePisc, CriaTipo.MAR),
                                                2000 * (piscifactorias.size() - 1));
                                        System.out.println("Piscifactoria añadida");
                                    } else {
                                        break;
                                    }
                        ...
                        case 2:
                            if (Monedero.monedasSuficientes(2000)) {
                                ...
                                this.registros.comprarEdificio(null, 2000);
                            ...
                            switch (op3) {
                                case 1:
                                    ...
                                    if (pisc.getTipo() == CriaTipo.RIO) {
                                        if (Monedero.monedasSuficientes(150) && pisc.getTanques().size() < 10) {
                                            ...
                                            this.registros.mejorarEdificio(pisc, 150, pisc.getTanques().indexOf(tanque),
                                                    null);
                                        ...
                                        if (Monedero.monedasSuficientes(600) && pisc.getTanques().size() < 10) {
                                            ...
                                            pisc.getTanques().add(tanque);
                                            this.registros.mejorarEdificio(pisc, 600, pisc.getTanques().indexOf(tanque),
                                                    null);
                                            System.out.println("Tanque añadido");
                                        }
                                    }
                                ...
                                case 2:
                                    ...
                                    if (Monedero.monedasSuficientes(50)) {
                                        ...
                                        this.registros.mejorarEdificio(pisc2, 50, -1, null);
                                    }
                                    break;
                                default:
                                    System.out.println("Esa opción no es válida.");
                            ...
                            switch (op3) {
                                case 1:
                                    if (Monedero.monedasSuficientes(200)) {
                                        ...
                                        this.registros.mejorarEdificio(null, 200, -1, almacenCentral);
                                    }
                                    break;
                                default:
                                    System.out.println("Esa opción no es válida.");
    ...
    /**
     * Método que devuelve la información de la librería de un tipo de pez
     * específico, pasado por parámetro.
     * 
        ...
        do {
            ...
            switch (opCant) {
                ...
                case 4:
                    if (almacenCentral == null) {
                        if (tipo == 0
                                && Monedero.monedasSuficientes(pisc.getMaxComidaAnimal() - pisc.getComidaAnimal())) {
                            cantComida = pisc.getMaxComidaAnimal() - pisc.getComidaAnimal();
                            return new int[] { cantComida, tipo };
                        } else if (tipo == 1
                                && Monedero.monedasSuficientes(pisc.getMaxComidaVegetal() - pisc.getComidaVegetal())) {
                            cantComida = pisc.getMaxComidaVegetal() - pisc.getComidaVegetal();
                            return new int[] { cantComida, tipo };
                        }
                    } else if (almacenCentral != null) {
                        if (tipo == 0 && Monedero.monedasSuficientes(
                                almacenCentral.getCapacidadComidaAnimal() - almacenCentral.getComidaAnimal())) {
                            cantComida = almacenCentral.getCapacidadComidaAnimal() - almacenCentral.getComidaAnimal();
                            return new int[] { cantComida, tipo };
                        } else if (tipo == 1 && Monedero.monedasSuficientes(
                                almacenCentral.getCapacidadComidaVegetal() - almacenCentral.getComidaVegetal())) {
    ...
    /**
     * Método que añade a una piscifactoría la cantidad del tipo de comida pasado
     * por parámetro
     * 
    ...
    public void anadirComidaPisc(Piscifactoria pisc) {
        ...
        this.registros.comprarComida(cant, tipo, precioComida, "piscifactoria", pisc);
    ...
    private void anadirComidaAlm() {
        ...
        if (tipo == "Vegetal") {
            ...
            this.registros.comprarComida(cant, tipo, precioComida, "almacen central", null);
            almacenCentral.repartir(piscifactorias);
            System.out.println("Deposito de comida vegetal del almacen central al "
                    + ((cant * 100 / almacenCentral.getCapacidadComidaVegetal()))
                    + "% de su capacidad. [ " + almacenCentral.getComidaVegetal() + "/"
                    + almacenCentral.getCapacidadComidaVegetal() + "]");
        } else if (tipo == "Animal") {
            almacenCentral.addFood(cant, tipo);
            this.registros.comprarComida(cant, tipo, precioComida, "almacen central", null);
            almacenCentral.repartir(piscifactorias);
            System.out.println("Deposito de comida animal del almacen central al "
                    + ((cant * 100 / almacenCentral.getCapacidadComidaAnimal()))
                    + "% de su capacidad. [ " + almacenCentral.getComidaAnimal() + "/"
                    + almacenCentral.getCapacidadComidaAnimal() + "]");
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                for (Tanque tanque : pisc.getTanques()) {
                    switch (randomNumber) {
                        case 1:
                            if (tanque.getTipoPez().getNombre() == "Lucio del norte"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new LucioDelNorte(false));
                            }
                            break;
                        case 2:
                            if (tanque.getTipoPez().getNombre() == "Carpa plateada"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new CarpaPlateada(false));
                            }
                            break;
                        case 3:
                            if (tanque.getTipoPez().getNombre() == "Carpa"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Carpa(false));
                            }
                            break;
                        case 4:
                            if (tanque.getTipoPez().getNombre() == "Tilapia del nilo"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new TilapiaDelNilo(false));
                            }
                            break;
                        case 5:
                            if (tanque.getTipoPez().getNombre() == "Pejerrey"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Pejerrey(false));
                            }
                            break;
                        case 6:
                            if (tanque.getTipoPez().getNombre() == "Rodaballo"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Rodaballo(false));
                            }
                            break;
                        case 7:
                            if (tanque.getTipoPez().getNombre() == "Caballa"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Caballa(false));
                            }
                            break;
                        case 8:
                            if (tanque.getTipoPez().getNombre() == "Besugo"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Besugo(false));
                            }
                            break;
                        case 9:
                            if (tanque.getTipoPez().getNombre() == "Abadejo"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Abadejo(false));
                            }
                            break;
                        case 10:
                            if (tanque.getTipoPez().getNombre() == "Sargo"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new Sargo(false));
                            }
                            break;
                        case 11:
                            if (tanque.getTipoPez().getNombre() == "Trucha arcoiris"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
                                tanque.getPeces().add(new TruchaArcoiris(false));
                            }
                            break;
                        case 12:
                            if (tanque.getTipoPez().getNombre() == "Dorada"
                                    && tanque.getMaxPeces() < tanque.getPeces().size()) {
            ...
            this.registros.ocultas(pisc, 0, 0);
        } catch (IndexOutOfBoundsException e) {
            System.out.println("Introduce un número válido, krak");
        }
    }
