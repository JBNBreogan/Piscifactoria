...
public class Simulador {
     ...
     * 
     * @param sim DTO del simulador
     */
    public Simulador(DTOSimulador sim) {
                    ...
                    if (Monedero.monedasSuficientes(pezEleg.getCoste())) {
                        if (pisc.pecesEnPiscifactoria() < pisc.pecesMaxPiscifactoria()) {
                            if (tank.getPeces().size() < 1) {
                                ...
                                if (pezEleg.isFemale()) {
                                    registros.comprarPeces(pezEleg, 'H', pisc.getTanques().indexOf(tank), pisc,
                                            pezEleg.getCoste());
                                } else {
                                    registros.comprarPeces(pezEleg, 'M', pisc.getTanques().indexOf(tank), pisc,
                                            pezEleg.getCoste());
                                }
                        ...
                        switch (op2) {
                            case 1:
                                ...
                                switch (op3) {
                                    case 1:
                                        System.out
                                                .println("Elige la piscifactoria a la que le quieres añadir un tanque");
                                        Piscifactoria pisc = piscifactorias.get(selectPisc());
            ...
            try {
                ...
                switch (opCant) {
                    ...
                    case 4:
                        if (almacenCentral == null) {
                            if (tipo == 0 && Monedero
                                    .monedasSuficientes(pisc.getMaxComidaAnimal() - pisc.getComidaAnimal())) {
                                cantComida = pisc.getMaxComidaAnimal() - pisc.getComidaAnimal();
                                return new int[] { cantComida, tipo };
                            } else if (tipo == 1 && Monedero
                                    .monedasSuficientes(pisc.getMaxComidaVegetal() - pisc.getComidaVegetal())) {
                            ...
                            if (tipo == 0 && Monedero.monedasSuficientes(
                                ...
                                cantComida = almacenCentral.getCapacidadComidaAnimal()
                                        - almacenCentral.getComidaAnimal();
                                return new int[] { cantComida, tipo };
                            } else if (tipo == 1 && Monedero.monedasSuficientes(
                                    almacenCentral.getCapacidadComidaVegetal() - almacenCentral.getComidaVegetal())) {
                                cantComida = almacenCentral.getCapacidadComidaVegetal()
                                        - almacenCentral.getComidaVegetal();
                                return new int[] { cantComida, tipo };
                            }
                        }
                        b = true;
    ...
    public void menuEnfermedades() {
        ...
        do {
            ...
            System.out.println("Quieres curar los peces de alguna piscifactoria?(Si/No)");
            resp = InputHelper.readStringWithBuffRead();
            if (resp.equals("Si")) {
                elegirPiscParaCurar();
                break;
            }
    ...
    /**
     * Método que permite elegir una piscifactoria de la que curar los peces.
     */
    public void elegirPiscParaCurar() {
        System.out.println("De que piscifactoria?");
        int op = InputHelper.getIntRanges(piscifactorias.size());
        Piscifactoria pisc = piscifactorias.get(op - 1);
        int monedasParaCurar=pisc.calcularMonedasCurar();
        System.out.println("Para curar los peces de esta piscifactoria son necesarias " +
        monedasParaCurar + " monedas.\nCurar a los peces? (Si/No)");
        String curar=InputHelper.readStringWithBuffRead();
        if(curar.equals("Si")){
            pisc.curarPeces();
            registros.curarPeces((monedasParaCurar/10), monedasParaCurar, pisc);
            monedero.setMonedas(monedero.getMonedas()-monedasParaCurar);
        }
    }
