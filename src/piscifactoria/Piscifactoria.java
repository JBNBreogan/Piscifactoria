...
/**
 * Clase que representa una piscifactoría, contiene uno o varios tanques de
 * peces, donde se gestiona todas las operaciones relacionados con ellos.
 * 
 * @author Nicolás
 */
...
public class Piscifactoria {
    ...
    /**
     * Objeto Monedero que se encarga de gestionar el sistema de monedas de la
     * piscifactoría.
     */
    ...
    public void showStatus() {
        ...
        int pecesAlimentadosPorcentaje = ((pecesVivosEnPisc != 0) ? ((pecesAlimentEnPisc * 100) / pecesVivosEnPisc)
                : 0);
        System.out.println("Peces alimentados: " + pecesAlimentEnPisc + "/" + pecesVivosEnPisc + "("
                + pecesAlimentadosPorcentaje + "%)");

        int pecesAdultosPorcentaje = (pecesVivosEnPisc != 0) ? ((peceAdultEnPisc * 100) / pecesVivosEnPisc) : 0;
        System.out.println(
                "Peces adultos: " + peceAdultEnPisc + "/" + pecesVivosEnPisc + "(" + pecesAdultosPorcentaje + "%)");
        ...
        int comidaAnimalPorcentaje = (this.maxComidaAnimal != 0) ? ((this.comidaAnimal * 100) / this.maxComidaAnimal)
                : 0;
        System.out.println("Almacén de comida animal: " + this.comidaAnimal + "/" + this.maxComidaAnimal + "("
                + comidaAnimalPorcentaje + "%)");

        int comidaVegetalPorcentaje = (this.maxComidaVegetal != 0)
                ? ((this.comidaVegetal * 100) / this.maxComidaVegetal)
                : 0;
        System.out.println("Almacén de comida vegetal: " + this.comidaVegetal + "/" + this.maxComidaVegetal + "("
                + comidaVegetalPorcentaje + "%)");
    }
    ...
    /**
     * Permite al usuario seleccionar un tanque con un pez en especifico(pasado por
     * parametro) de la piscifactoria.
     * 
    ...
    /**
     * Avanza un día en la piscifactoría, actualizando los tanques y el estado de
     * los peces.
     */
    public int[] nextDay(estadisticas.Estadisticas stats) {
        ...
        for (Tanque tanque : tanques) {
            int[] currTankValues = tanque.nextDay(this, stats, this.tanquesHuevos);
            totalmonedastanques += currTankValues[0];
            totalpeces += currTankValues[1];
        }
        for (TanqueHuevos tanqueHuevos : tanquesHuevos) {
            tanqueHuevos.nextDay(tanques);
        }

        for (TanqueCria tanqueCria : tanquesCria) {
            Pez[] pezCria = tanqueCria.getPez();

            if (pezCria[0] != null) {
                if (pezCria[0] instanceof Carnivoro) {
                    if (comidaAnimal >= 2) {
                        tanqueCria.nextDay(comidaAnimal);
                        comidaAnimal -= 2;
                    }
                } else if (pezCria[0] instanceof Filtrador) {
                    if (comidaVegetal >= 2) {
                        tanqueCria.nextDay(comidaVegetal);
                        comidaVegetal -= 2;
                    }
                } else if (pezCria[0] instanceof Omnivoro) {
                    if (comidaAnimal >= 1 && comidaVegetal >= 1) {
                        tanqueCria.nextDay(Math.min(comidaAnimal, comidaVegetal));
                        comidaAnimal -= 1;
                        comidaVegetal -= 1;
                    }
                }
        ...
        System.out.println("Piscifactoria " + getNombre() + ": " + totalpeces + " peces vendidos por "
                + totalmonedastanques + " monedas");
        int[] retorno = { totalmonedastanques, totalpeces };
        return retorno;
    }

    /**
     * Vende los peces adultos y vivos de la piscifactoría, actualizando las
     * monedas.
     */
    ...
    public void upgradeFood() {
        ...
        System.out.println(
                "Almacén de comida de la piscifactoría " + this.nombre + " mejorado. Su capacidad ha aumentado en "
                        + ((tipo == CriaTipo.RIO) ? "25" : "100") + " hasta un total de " + maxComidaAnimal);
    }
    ...
    /**
     * Vende los peces más óptimos (adultos y vivos) y devuelve el número total de
     * peces vendidos
     * y la cantidad de monedas obtenidas.
     * 
     * @return Un array de dos enteros: la cantidad de monedas y el número de peces
     *         vendidos.
     */
